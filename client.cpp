...
// Указываем линковщику использовать библиотеку сокетов
...
bool isValidInteger(const string& s) {
    ...
    if (s[0] == '-') { //Если число отрицательное - пропускаем минус и начинаем со следующего символа
        start = 1; 
        if (s.length() == 1) return false; //Если строка состоит только из минуса - возвращаем false
    }
...
T getSafeInput() { //Запускаем фунцию в зависимости от типа данных
...
int main(void) {
    // 1. Настройка консоли, для вывода используем локаль Win1251
    SetConsoleCP(1251);
    ...
    const char SERVER_IP[] = "10.77.129.244"; //IPv4-адрес сервера класса А 
    const short SERVER_PORT_NUM = 8081; //порт сервера
    
    // 3. Инициализация WinSock
    WSADATA wsData;
    if (WSAStartup(MAKEWORD(2, 2), &wsData) != 0) { //Используем версию WinSock 2.2
        cerr << "Critical Error: WinSock startup failed: " << WSAGetLastError() << endl; 
        return 1;
    }
    ...
    ZeroMemory(&servInfo, sizeof(servInfo)); //Очищаем servInfo от мусора, забивая её нулями
    ...
    while (userdata != 1 && userdata != 2) {
        ...
        if (userdata == 1) {
            ...
            if (start > end) std::swap(start, end); //Проверка на дурака: если начальное значение интервала больше конечного - меняем их местами
    ...
    file.close(); //Закрываем файл 
    if (!validDataString.empty()) validDataString.pop_back(); //Убираем лишний пробел в конце
    ...
    for (char c : fullPacket) {
        ...
        if (++printCount >= 50) break; // Ограничитель
    }
    cout << "\n" << endl;
    // ========================================
    ...
    shutdown(ClientSock, SD_SEND); //Заканчиваем отправку данных и говорим об этом серверу
    ...
    while (true) {
        ...
        if (receivedNow > 0) totalReceived += receivedNow; //Увеличиваем totalReceived, если получили какие-то данные, если нет - выходим из цикла получения
        else break;
    }
