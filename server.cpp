...
pair<bool, vector<float>> read_nums_from_socket(int socket)
{
    ...
    cout << "Из сокета считано " << count_read << " чисел" << endl;
    if (count_read != 0) {
        result.first = true;
    } else {
        cerr << "Пользователь отправил пустой файл, закрываем сокет" << endl;
        result.first = false;
    }
    result.second = nums;
    return result;
...
int main()
{
    ...
    while (1) {
        ...
        if (algorithm == 1) {
            ...
            result = shell_fvec_sort(nums);
            cout << "Сортировка вставками" << endl;
            vector<float> insertions = insertions_fvec_sort(nums);
            // Формируем вектор с возвращаемыми данными в виде
            // сравнения -> перестановки -> время (для Шелла) -> сравнения -> перестановки -> время (для вставки) -> отсортированный массив
            result.insert(result.begin() + 3, insertions.begin(), insertions.begin() + 3);
        } else if (algorithm == 3) {
            // Вызываем функцию сортировки (6 вариант)
            cout << "Выбран алгоритм сортировки (2)" << endl;
            // Клонируем массивы, так как Леха реализовывал сортировку не через передачу значений а через адреса
            vector<float> nums_fast = nums;
            vector<float> nums_choices = nums;
            cout << "Быстрая сортировка" << endl;
            result = quickSort(nums_fast);
            cout << "Сортировка вставками" << endl;
            vector<float> choices = insertions_fvec_sort(nums_choices);
            // Формируем вектор с возвращаемыми данными в виде
            // сравнения -> перестановки -> время (для быстрой) -> сравнения -> перестановки -> время (для сортировки выбором) -> отсортированный массив
            result.insert(result.begin() + 3, choices.begin(), choices.begin() + 3);
        } else {
